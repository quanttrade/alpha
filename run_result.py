...
if __name__ == '__main__':
    ...
    alpha_corr = alpha_corr = pd.read_csv(
        'D:data/daily_data/alpha_corr.csv', index_col=0)
    ...
    benchmark_component = pd.read_csv(
        'D:data/daily_data/zz500_component.csv', index_col=0)
    benchmark_component.index = pd.DatetimeIndex(benchmark_component.index)
    weight_bound = 0.05
    risk_loading_bound = 0.01
    industry_loading_bound = 0.05
    TC_list = [0.003, 0.005, 0.004, 0.006, 0.007, 0.008, 0.009, 0.01]
    ...
    for period in periods[4:]:
        ...
        for ir_limit in IR_limit:
            ...
            factor_returns, rsquare, resid_returns = caculate_factor_returns(
                total_factor, price_data, period)
            factor_returns.to_hdf(
                os.path.join(
                    ir_path,
                    'factor_returns.h5'),
                'table')
            ...
            for TC in TC_list:
                ...
                cumulative_return, position = alpha_model_backtest(risk_factors, industry_factor, alpha_factor, alpha_returns,
                                                                   ...
                                                                   industry_loading_bound, TC, period, 120)
...
#                turnover = turnover[turnover>0]
                cumulative_excess_return.to_csv(os.path.join(
                    tc_path, 'cumulative_excess_return.csv'))
                ret_situation.to_csv(
                    os.path.join(
                        tc_path,
                        'ret_situation.csv'))
